...
export default function PromptManagement() {
  ...
  const deletePrompt = async (promptId: string) => {
    ...
    toast.confirm(
      `Are you sure you want to delete the prompt?.This action cannot be undone!`,
      async () => {
        try {
      ...
      <>
        ...
        {isSuperAdmin && !isEditing && (
          <div className="flex items-center gap-4 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
            ...
            <select
              className="max-w-xs w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            {selectedClientId && (
              <span className="text-xs font-medium text-amber-600 bg-amber-50 px-2 py-1 rounded-full border border-amber-100 animate-pulse">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-6 mb-6 shadow-md">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t">
                <label className="block text-sm font-medium text-slate-700 mb-2 text-xs uppercase">
                  Change Notes
                </label>
                <input
                  type="text"
            ...
            {!isEditing && (
              <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-slate-600">
                  All prompt edits are automatically versioned. The system uses
                  the latest version for facet generation. Mark prompts with
                  Level 2-10 as master templates for comprehensive facet
                  building.
                </p>
              </div>
            )}
              ...
              {isLoadingHistory ? (
                ...
                <div className="space-y-6">
                  {promptVersions.length > 0 && (
                    <div
                      className={`mb-4 px-4 py-3 rounded-lg border flex items-center gap-3 ${
