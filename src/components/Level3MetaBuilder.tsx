...
import { Project, MetaLevel3 } from '../types';

interface Level3MetaBuilderProps {
  project: Project;
  onComplete: () => void;
}

export default function Level3MetaBuilder({ project, onComplete }: Level3MetaBuilderProps) {
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [metaData, setMetaData] = useState<MetaLevel3 | null>(null);
  ...
  const loadExistingMeta = async () => {
    ...
    if (data) {
      setMetaData(data as MetaLevel3);
    }
  };

  const analyzeLevel3 = async () => {
    setIsAnalyzing(true);
    setError('');

    try {
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/analyze-level3`;
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          l3_category: project.l3_category,
        }),
      });
  ...
  if (!project.l3_category) {
    return (
      <div className="max-w-2xl mx-auto text-center">
        <p className="text-slate-600 mb-4">
          No Level 3 category specified for this project.
        </p>
        <button
          onClick={onComplete}
          className="px-8 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"
        >
          Skip to Facet Builder
        </button>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-semibold text-slate-900 mb-2">
            Level 3 Meta Analysis Complete
          </h2>
  ...
  return (
    <div className="max-w-2xl mx-auto text-center">
      ...
      <p className="text-slate-600 mb-8">
        Analyze end-level purchasable category SEO patterns for: <strong>
          {project.l1_category}
          {project.l2_category && ` > ${project.l2_category}`}
          {project.l3_category && ` > ${project.l3_category}`}
        </strong>
