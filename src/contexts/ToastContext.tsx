...
import { CheckCircle, AlertCircle, X, Info, AlertTriangle, Check } from 'lucide-react';
...
function ToastContainer({ toasts, dismiss }: { toasts: Toast[]; dismiss: (id: string) => void }) {
  ...
  const styles = {
    success: { icon: CheckCircle, border: 'border-green-500/50', bg: 'bg-slate-900/95', text: 'text-green-400', glow: 'shadow-green-500/20' },
    error: { icon: AlertCircle, border: 'border-red-500/50', bg: 'bg-slate-900/95', text: 'text-red-400', glow: 'shadow-red-500/20' },
    warning: { icon: AlertTriangle, border: 'border-yellow-500/50', bg: 'bg-slate-900/95', text: 'text-yellow-400', glow: 'shadow-yellow-500/20' },
    info: { icon: Info, border: 'border-blue-500/50', bg: 'bg-slate-900/95', text: 'text-blue-400', glow: 'shadow-blue-500/20' },
    confirm: { icon: AlertTriangle, border: 'border-purple-500/50', bg: 'bg-slate-900/95', text: 'text-purple-400', glow: 'shadow-purple-500/20' },
  };
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center justify-center pointer-events-none">
      <div 
        className={`
          pointer-events-auto
        ...
        <div className="flex-1">
         ...
         {t.type === 'confirm' ? (
          <div className="flex gap-2 ml-2">
            <button
              onClick={() => {
                // Call onCancel if it exists, otherwise just dismiss
      ...
      {t.type !== 'confirm' && (
         <div className="w-full h-1 mt-2 overflow-hidden rounded-full max-w-[300px]">
             <div className="h-full bg-white/20 animate-shrink-width" style={{ animationDuration: '4s' }}></div>
         </div>
      )}
    </div>
  );
